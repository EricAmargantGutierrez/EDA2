...
void configure_character(Character *player) {
    ...
    switch (choice) {
        case 1:
            ...
            memcpy(player->skills, temp_skills, sizeof(player->skills));
            break;
        case 2:
...
// Initialize enemies
Enemy deciums = {"Decimus Brutus", 80, 0.4, 0.5, skills}; //cambiar pq no puguin fer servir totes les skills
...
Scenario rome_dec, pompeii_dec, thermal_dec, colosseum_dec;
//scenario 1 decision:



init_option(Option* option, char* initial_text, char* response, char* final_text);
init_option(Option* option, char* initial_text, char* response, char* final_text);
